...
function generateInfo(classname, dept, term, cb) {
  query.requestSection(classname, dept, term, function(err,sectionArray) {
    if(err){
      return cb(err);
    }
    var setInfo = {};
...
function createSectionsArray(name, term, cb) {
  ...
  dept = name.split('-')[0];
  var answer = [];
  generateInfo(name, dept, term, function(err,setInfo, sectionArray) {
    if(err){
      return cb(err);
    }
